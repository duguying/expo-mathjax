 ...
 */
export const COMMON_PACKAGES: string[] = [
    "base",
    "ams",
    "newcommand",
    "boldsymbol",
...
const TEX_PKG_LOADERS: Record<string, () => any> = {
...
let _configuredPackages: string[] = COMMON_PACKAGES;
...
export function configure(opts: ConfigureOptions): void {
    if (_initialized) return; // too late – engine already started
    if (opts.packages) _configuredPackages = opts.packages;
}
...
const MAX_CACHE = 128;
const _cache = new Map<string, SvgData>();

function cacheKey(mode: string, input: string, display: boolean): string {
 ...
 */
function parseExValue(attr: string | null): number {
    if (!attr) return 0;
    const m = attr.match(/^([\d.eE+-]+)(ex|em)?/i);
    if (!m) return 0;
    const num = parseFloat(m[1]);
    if (!isFinite(num)) return 0;
    return (m[2] ?? "").toLowerCase() === "em" ? num * 2 : num;
}
 ...
 */
function buildSvgData(svgEl: any): SvgData {
    ...
    let svgString: string = _adaptor.outerHTML(svgEl);
...
function setInCache(key: string, data: SvgData): void {
 ...
 */
export function texToSvg(tex: string, opts: ConvertOptions = {}): SvgData {
    ...
    if (!svgEl) throw new Error(`MathJax produced no SVG for: ${tex}`);
...
export function mathmlToSvg(
    ...
    const svgEl = container.children?.[0];
    if (!svgEl) throw new Error(`MathJax produced no SVG for MathML input`);
